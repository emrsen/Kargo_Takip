...
void GrapGPS() {
  ...
  if (gpsFix) {
    ...
    /*String SAAT = gpsbuffer1.substring(12,18);
    Serial.print("saat ");
    Serial.println(SAAT);
    ...
    Serial.print("DK ");
    Serial.println(DK);
    String Gun = tarih.substring(6,8);
    Serial.print("Gun ");
    Serial.println(Gun);
    if ((DK == Eski_DK) && (Gun == Eski_Gun)){  // DK ve Gun degerleri aynı ise kayıt atmıyoruz
        Serial.println("Ayni dk icindeyiz");
      } 
    else {                                      // DK veya Gun degerlerinden birisi ya da ikisi de degistiyse işlem yapılması lazım
      if (Gun == Eski_Gun) {                    // Gun değeri aynı ise SD kart üzerindeki dosyaya ekleme yapılıyor
        Serial.println("!!!DAKIKA DEGISTI!!!");
          //if (SD_Ekle(tarih, gpsbuffer1)) {              // işlem başarı ise Eski_DK, DK olarak değiştirilir
          Eski_DK = DK;
        //}
      } 
      /*else {                                    // Gun değeri değişmiş ise 
        //if SD_YeniGun(gpsbuffer1) {           // SD kart üzerinde yeni tarihli dosya oluşturuluyor
          Eski_Gun = Gun;                     // İşlem başarı ise Eski_Gun, Gun olarak değiştirilir
        //}
        //if SD_Ekle(gpsbuffer1) {              // işlem başarı ise Eski_DK, DK olarak değiştirilir
          Eski_DK = DK;
        //}
      }*/
    }
